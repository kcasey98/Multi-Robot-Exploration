function [vorvx,pos] = voronoi(pos0,bnd_pnts,n,ij)
...
%         figure('position',[0 0 600 600],'Color',[1 1 1]);
        for i = 1:size(pos,1)
        K = convhulln(vorvx{i});
        trisurf(K,vorvx{i}(:,1),vorvx{i}(:,2),'FaceColor',col(i,:),'FaceAlpha',0.5,'EdgeAlpha',1)
        hold on;
        end
